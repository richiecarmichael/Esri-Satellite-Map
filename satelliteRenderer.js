define([
    ...
    return declare([], {
        constructor: function (satellites) {
            ...
            // Cross-reference between the satellite array and webgl.
            this.index = [];
        },
        ...
        _createObjects: function () {
            ...
            this.normal.geometry.addAttribute('position', new THREE.Float32Attribute(vertex, 3));
        ...
        _getSatelliteLocation: function (satrec, date) {
        ...
        mousemove: function (x, y) {
            ...
            if (intersections.length !== 0) {
                ...
                satellite = this.satellites[this.index[index]];
            }
        ...
        showOrbit: function () {
            ...
            var ms = this.satelliteHover.metadata.period * 60000;
            ms /= TRAJECTORY_SEGMENTS;

            // Show orbit
            var vertex = [];
            for (var i = 0; i <= TRAJECTORY_SEGMENTS ; i++) {
                var date = new Date(this.date.valueOf() + i * ms);
                var eci = this._getSatelliteLocation(this.satelliteHover.satrec, date);
                if (eci === null || eci === undefined || isNaN(eci.x) || isNaN(eci.y) || isNaN(eci.z)) {
                    continue;
                }
                vertex.push(
